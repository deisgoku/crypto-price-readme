...
const renderClassic = (data, theme = 'dark', limit = 10, categoryText = 'Global') => {
  const {
  ...
  const rows = data.slice(0, limit).map((coin, i) => {
    ...
    const highlightWidth = xCols[xCols.length - 1] + 130 - xCols[0];

    return `
      <g transform="translate(0, ${y})">
        <rect x="${xCols[0]}" y="0" width="${highlightWidth}" height="${rowHeight}" fill="${rowFill}" />
        ${xCols.map((x) => `
