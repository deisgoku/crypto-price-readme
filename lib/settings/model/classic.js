...
const renderClassic = (data, theme = 'dark', limit = 10) => {
  ...
  const tableWidth = svgWidth - tableMargin * 2;
  ...
  const rows = data.slice(0, limit).map((coin, i) => {
    ...
    return `
      <g transform="translate(0, ${y})">
        <rect x="${tableMargin}" y="0" width="${tableWidth}" height="${rowHeight}" fill="${rowFill}" />
        ${xCols.map((x) => `
        ...
        <g transform="translate(${xCols[4] + 15}, 8)">
          <path d="${coin.chart}" fill="none" stroke="#0ff" stroke-width="2" transform="scale(0.85, 0.85)" />
        </g>
      </g>
    `;
  }).join('');
