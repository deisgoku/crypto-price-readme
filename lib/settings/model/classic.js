...
const renderClassic = (data, theme = 'dark', limit = 10) => {
  ...
  const rows = data.slice(0, limit).map((coin, i) => {
    ...
    const rowFill = i % 2 === 0 ? '#ffffff22' : '#00bfff22'; // putih & biru terang transparan
    ...
    return `
      <g transform="translate(0, ${y})">
        ...
        <g transform="translate(${xCols[4] + 20}, 8)">
          <path d="${coin.chart}" fill="none" stroke="${trendColor}" stroke-width="2" transform="scale(0.8, 0.8)" />
        </g>
      </g>
    `;
  }).join('');
