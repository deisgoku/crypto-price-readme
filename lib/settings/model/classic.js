...
const sanitizePath = (d) => String(d).replace(/[<>"']/g, '');

const renderClassic = (data, theme = 'dark', limit = 10) => {
  console.log('RenderClassic called with theme:', theme);
  ...
  const rows = data.slice(0, limit).map((coin, i) => {
    console.log(`Processing coin #${i + 1}:`, coin.symbol);
    ...
    // Validasi chart path
    const rawChart = String(coin.chart || '');
    const isValidChart = /^[MLCQZ0-9 .\-]+$/i.test(rawChart);
    const chartPath = isValidChart ? sanitizePath(rawChart) : 'M0 10 L100 10';

    if (!isValidChart) {
      console.warn(`Invalid chart path for ${coin.symbol}, using fallback.`);
    }
    ...
    return `
      <g transform="translate(0, ${y})">
        ...
        <g transform="translate(${xCols[4] + 20}, 8)">
          <rect x="0" y="0" width="100" height="24" fill="none" stroke="red" stroke-width="0.3" />
          <path d="${chartPath}" fill="none" stroke="${trendColor}" stroke-width="2" transform="scale(0.8, 0.8)" />
        </g>
      </g>
    `;
  }).join('');
  ...
  console.log('Final SVG height:', height);
