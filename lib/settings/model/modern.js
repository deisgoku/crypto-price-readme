...
const renderModern = (data, theme = 'dark', limit = 6) => {
  ...
  const tableHead = `
    ...
    <text x="${xTrend + (maxTrendLength * charWidth)}" y="${tableHeadY + 15}" ${font} fill="${headText}" text-anchor="end" font-weight="bold">Trend</text>
...
const renderLegacy = (data, theme = 'dark', limit = 6) => {
  return renderModern(data, theme, limit).replace('v1.5.5 (PRO)', 'v1.4.1');
};

module.exports = { renderModern, renderLegacy };
