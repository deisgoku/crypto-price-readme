...
const renderModern = (data, theme = 'dark', limit = 6) => {
  ...
  // Posisi kolom dengan text-anchor: middle
  const xName = 80;
  const xPrice = 200;
  const xVol = 320;
  const xTrend = 440;
  ...
  const rows = data.slice(0, limit).map((coin, i) => {
    ...
    return `
      <g transform="translate(20, ${y})">
        ...
        <text x="${xName}" y="25" ${font} fill="${textColor}" text-anchor="middle">${coin.symbol}</text>
        <text x="${xPrice}" y="25" ${font} fill="${textColor}" text-anchor="middle">$${coin.price}</text>
        <text x="${xVol}" y="25" ${font} fill="${textColor}" text-anchor="middle">${coin.volume}</text>
        <text x="${xTrend}" y="25" ${font} fill="${trendColor}" text-anchor="middle">${coin.trend.toFixed(2)}% ${trendIcon}</text>
