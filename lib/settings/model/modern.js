...
const renderModern = (data, theme = 'dark', limit = 6) => {
  ...
  const tableHead = `
    <g transform="translate(0, 60)">
      <rect x="10" y="0" width="580" height="${headerHeight}" rx="${borderRadius}" fill="${headBg}" stroke="${rowBorder}" />
  ...
  const rows = data.slice(0, limit).map((coin, i) => {
    ...
    return `
      <g transform="translate(10, ${y})">
        <rect width="580" height="50" rx="${borderRadius}" fill="${rowBg}" stroke="${rowBorder}" />
  ...
  <svg width="${svgWidth}" height="${height}" viewBox="0 0 ${svgWidth} ${height}" xmlns="http://www.w3.org/2000/svg">
    ...
    <rect x="10" y="5" width="${svgWidth - 20}" height="${height - 10}" fill="${bgColor}" rx="14" filter="url(#shadow)" />
    ${header}
    ${tableHead}
    ${rows}
    ${footer}
