...
const renderModern = (data, theme = 'dark', limit = 6) => {
  ...
  const tableHead = `
    <g transform="translate(20, 80)">
      ...
      <text x="${xName}" y="25" ${font} fill="${headText}" text-anchor="middle" font-weight="bold">NAME</text>
      <text x="${xPrice}" y="25" ${font} fill="${headText}" text-anchor="middle" font-weight="bold">PRICE</text>
      <text x="${xVol}" y="25" ${font} fill="${headText}" text-anchor="middle" font-weight="bold">VOL</text>
      <text x="${xTrend}" y="25" ${font} fill="${headText}" text-anchor="middle" font-weight="bold">%24h</text>
      <text x="${xChart}" y="25" ${font} fill="${headText}" text-anchor="middle" font-weight="bold">CHART</text>
    </g>`;
  ...
  const rows = data.slice(0, limit).map((coin, i) => {
    ...
    return `
      <g transform="translate(20, ${y})">
        ...
        <path d="${coin.chart}" fill="none" stroke="#0ff" stroke-width="2" transform="translate(${xChart - chartWidth / 2},10) scale(0.75,0.8)" />
      </g>`;
  }).join('');
