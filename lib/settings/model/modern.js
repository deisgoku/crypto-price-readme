...
const renderModern = (data, theme = 'dark', limit = 6) => {
  ...
  const innerWidth = xChart + chartWidth + 40;
  ...
  const tableHeadY = 50;
  const tableHead = `
    <rect x="20" y="${tableHeadY}" width="${innerWidth - 40}" height="${headerHeight}" rx="${borderRadius}" fill="${headBg}" stroke="${rowBorder}" />
  ...
  const rows = data.slice(0, limit).map((coin, i) => {
    ...
    return `
      <rect x="20" y="${y - rowPaddingY}" width="${innerWidth - 40}" height="${rowHeight - 10}" rx="${borderRadius}" fill="${rowBg}" stroke="${rowBorder}" />
  ...
  return `
    <svg width="${innerWidth + 40}" height="${innerHeight + 40}" viewBox="0 0 ${innerWidth + 40} ${innerHeight + 40}" xmlns="http://www.w3.org/2000/svg">
