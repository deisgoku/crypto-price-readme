...
const { themes, trendDownGradient, trendUpGradient } = require('./theme');
...
const renderAurora = (data, theme = 'tokyonight', limit = 10) => {
  ...
  const rows = data.slice(0, limit).map((coin, i) => {
    ...
    const trendGradientId = coin.trend > 0 ? 'url(#auroraTrendUp)' : coin.trend < 0 ? 'url(#auroraTrendDown)' : 'transparent';
    const rowBg = trendGradientId;
