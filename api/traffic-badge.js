...
module.exports = async (req, res) => {
  ...
  try {
    ...
    //
    const redisUserCount = await redis.get(key);

    if (redisUserCount) {
      // 
      console.log(`[USER BADGE] Retrieved user count from Redis: ${redisUserCount}`);
      return generateBadge(res, redisUserCount);
    }
...
// Fungsi untuk menghasilkan badge dengan data user count
