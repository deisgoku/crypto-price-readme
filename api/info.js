...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const tableHeader = `
      ...
      <!-- Baris Header Utama -->
      <rect x="0" y="${headerY}" width="${colWidth[0] + colWidth[1]}" height="${rowHeight * 2}" rx="8" ry="8" fill="url(#aurora)" />
      <rect x="${colWidth[0] + colWidth[1]}" y="${headerY}" width="${colWidth[2]}" height="${rowHeight * 2}" rx="8" ry="8" fill="url(#aurora)" />
      ...
      <!-- Sub Header Themes -->
      <rect x="0" y="${headerY + rowHeight * 2}" width="${colWidth[0]}" height="${rowHeight}" fill="${headBg}" />
      <rect x="${colWidth[0]}" y="${headerY + rowHeight * 2}" width="${colWidth[1]}" height="${rowHeight}" fill="${headBg}" />
      
      <text x="10" y="${headerY + rowHeight * 2 + 20}" fill="${headText}" ${font}>${escapeXml("Theme 1")}</text>
      <text x="${colWidth[0] + 10}" y="${headerY + rowHeight * 2 + 20}" fill="${headText}" ${font}>${escapeXml("Theme 2")}</text>
    `;
